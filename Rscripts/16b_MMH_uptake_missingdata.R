...
write.table(outputs, paste0(results, "log_regression_coverage_updated.csv"), sep = "\t", row.names = T,
            dec = ".")
...
write.table(outputs, paste0(results, "log_regression_coverage_diagnoses.csv"), sep = "\t", row.names = T,
            dec = ".")
...
write.table(outputs, paste0(results, "log_regression_coverage_prescriptions.csv"), sep = "\t", row.names = T,
            dec = ".")
...
#fully adjusted model
DTP1_early <- glm(vaccinated_1y ~ MHI_vacc1_ex + ethnicity_5 + region + imd + 
                      mat_age + birth_order_cat + flurisk+ gender_child, data = earlydf1, family = "binomial")
PCV3_early <- glm(vaccinated_2y ~ MHI_vacc2_ex + ethnicity_5 + region + imd + 
              mat_age + birth_order_cat + flurisk+ gender_child, data = earlydf2, family = "binomial")
MMR2_early <- glm(vaccinated_5y ~ MHI_vacc3_ex + ethnicity_5 + region + imd + 
                    mat_age + birth_order_cat + flurisk+ gender_child, data = earlydf3, family = "binomial")

#late estimates
DTP1_late <- glm(vaccinated_1y ~ MHI_vacc1_ex + ethnicity_5 + region + imd + 
                    mat_age + birth_order_cat + flurisk+ gender_child, data = latedf1, family = "binomial")
PCV3_late <- glm(vaccinated_2y ~ MHI_vacc2_ex + ethnicity_5 + region + imd + 
                    mat_age + birth_order_cat + flurisk+ gender_child, data = latedf2, family = "binomial")
MMR2_late <- glm(vaccinated_5y ~ MHI_vacc3_ex + ethnicity_5 + region + imd + 
...
DTP1_year <- glm(vaccinated_1y ~ MHI_vacc1_ex + ethnicity_5 + region + imd + 
                   mat_age + birth_order_cat + flurisk+ gender_child + cal_year, data = df1, family = "binomial")
PCV3_year <- glm(vaccinated_2y ~ MHI_vacc2_ex + ethnicity_5 + region + imd + 
                   mat_age + birth_order_cat + flurisk+ gender_child + cal_year, data = df2, family = "binomial")
MMR2_year <- glm(vaccinated_5y ~ MHI_vacc3_ex + ethnicity_5 + region + imd + 
...
write.table(output, paste0(results, "log_regression_coverage_calendar_year.csv"), sep = "\t", row.names = T,
            dec = ".")
...
#sibling
DTP1_sib <- lme4::glmer(vaccinated_1y ~ MHI_vacc1_ex + ethnicity_5 + region + imd + 
                   mat_age + birth_order_cat + flurisk+ gender_child + (1|patid), 
                   data = df1, family = "binomial", nAGQ=0)
PCV3_sib <- lme4::glmer(vaccinated_2y ~ MHI_vacc2_ex + ethnicity_5 + region + imd + 
                   mat_age + birth_order_cat + flurisk+ gender_child + (1|patid),
                   data = df2, family = "binomial", nAGQ=0)
MMR2_sib <- lme4::glmer(vaccinated_5y ~ MHI_vacc3_ex + ethnicity_5 + region + imd + 
                   mat_age + birth_order_cat + flurisk+ gender_child + (1|patid),
                   data = df3, family = "binomial", nAGQ=0)

#GP
DTP1_gp <- lme4::glmer(vaccinated_1y ~ MHI_vacc1_ex + ethnicity_5 + region + imd + 
                          mat_age + birth_order_cat + flurisk+ gender_child + (1|pracid), 
                        data = df1, family = "binomial", nAGQ=0)
PCV3_gp <- lme4::glmer(vaccinated_2y ~ MHI_vacc2_ex + ethnicity_5 + region + imd + 
                          mat_age + birth_order_cat + flurisk+ gender_child + (1|pracid),
                        data = df2, family = "binomial", nAGQ=0)
MMR2_gp <- lme4::glmer(vaccinated_5y ~ MHI_vacc3_ex + ethnicity_5 + region + imd + 
                          mat_age + birth_order_cat + flurisk+ gender_child + (1|pracid),
                        data = df3, family = "binomial", nAGQ=0)


#both
DTP1_rand <- lme4::glmer(vaccinated_1y ~ MHI_vacc1_ex + ethnicity_5 + region + imd + 
                          mat_age + birth_order_cat + flurisk+ gender_child + 
                          (1|patid) + (1|pracid), 
                        data = df1, family = "binomial", nAGQ=0)
PCV3_rand <- lme4::glmer(vaccinated_2y ~ MHI_vacc2_ex + ethnicity_5 + region + imd + 
                          mat_age + birth_order_cat + flurisk+ gender_child +
                          (1|patid) + (1|pracid),
                        data = df2, family = "binomial", nAGQ=0)
MMR2_rand <- lme4::glmer(vaccinated_5y ~ MHI_vacc3_ex + ethnicity_5 + region + imd + 
                          mat_age + birth_order_cat + flurisk+ gender_child + 
                          (1|patid) + (1|pracid),
                        data = df3, family = "binomial", nAGQ=0)
...
write.table(outputs, paste0(results, "log_regression_coverage_glm_final.csv"), sep = "\t", row.names = T,
            dec = ".")
...
#crude models
model_crude_DTP<- lme4::glmer(vaccinated_1y ~ MHI_vacc1_ex + (1|patid) + (1|pracid),
                                    data = df1, family = "binomial", nAGQ=0) #236042.5  124466.5 
model_crude_PCV<- lme4::glmer(vaccinated_2y ~ MHI_vacc2_ex + (1|patid) + (1|pracid),
                              data = df2, family = "binomial", nAGQ=0) #199980.8 188022.2 
model_crude_MMR<- lme4::glmer(vaccinated_5y ~ MHI_vacc3_ex + (1|patid) + (1|pracid),
...
model1_DTP<- lme4::glmer(vaccinated_1y ~ MHI_vacc1_ex + region + ethnicity_5 + 
                              imd +(1|patid)+ (1|pracid),
                              data = df1, family = "binomial", nAGQ=0)
model1_PCV<- lme4::glmer(vaccinated_2y ~ MHI_vacc2_ex +  region + ethnicity_5 + 
                              imd +(1|patid)+ (1|pracid),
                              data = df2, family = "binomial", nAGQ=0)
model1_MMR<- lme4::glmer(vaccinated_5y ~ MHI_vacc3_ex +  region + ethnicity_5 + 
                              imd + (1|patid)+ (1|pracid),
                              data = df3, family = "binomial", nAGQ=0)
...
write.table(outputs, paste0(results, "log_regression_coverage_glm.csv"), sep = "\t", row.names = T,
            dec = ".")


#fully adjusted model
model2_DTP<- lme4::glmer(vaccinated_1y ~ MHI_vacc1_ex + region + ethnicity_5 + 
                           imd + mat_age + birth_order_cat + gender_child +
                           flurisk +  (1|patid)+ (1|pracid),
                         data = df1, family = "binomial", nAGQ=0)
model2_PCV<- lme4::glmer(vaccinated_2y ~ MHI_vacc2_ex +  region + ethnicity_5 + 
                           imd + mat_age + birth_order_cat + gender_child +
                           flurisk + (1|patid)+ (1|pracid),
                         data = df2, family = "binomial", nAGQ=0)
model2_MMR<- lme4::glmer(vaccinated_5y ~ MHI_vacc3_ex +  region + ethnicity_5 + 
                           imd + mat_age + birth_order_cat + gender_child +
                           flurisk + (1|patid)+ (1|pracid),
                         data = df3, family = "binomial", nAGQ=0)
...
write.table(outputs, paste0(results, "log_regression_coverage_glm_appendic.csv"), sep = "\t", row.names = T,
            dec = ".")
