...
tmp[age_birth < 20, mat_age := "<20"]
tmp[age_birth >= 20 & age_birth <30, mat_age := "20-29"]
tmp[age_birth >= 30 & age_birth <40, mat_age := "30-39"]
tmp[age_birth >=40, mat_age := ">40"]
tmp[, mat_age:= as.factor(mat_age)]

tmp[, birth_order_cat := factor(birth_order_cat)]
tmp[, mat_age := relevel(mat_age, ref = "20-29")]
tmp<- tmp[age_startfu_baby <= age_endfu_baby]
length(unique(tmp$babypatid)) #397518
length(unique(tmp$patid)) #330198
tmp <- tmp[gender_child != 3] #two children + mums
tmp <- tmp[ethnicity_5 != "Unknown"]
length(unique(tmp$babypatid)) #387338
length(unique(tmp$patid)) #321731
...
write.table(results_DTP, paste0(results, "aft_results_DTP_timingCMD_CC.csv"), sep = "\t", row.names = T,
            dec = ".")
...
results_PCV <- data.table(rbind(mod1, mod2, mod3))
results_PCV[, TR:= paste0(round(1/Estimate, digits = 2), " (", round(1/uci, digits = 2), "-", round(1/lci, digits = 2), ")")]

write.table(results_PCV, paste0(results, "aft_results_PCV_timingCMD_CC.csv"), sep = "\t", row.names = T,
            dec = ".")
...
results_MMR <- data.table(rbind(mod1, mod2, mod3))
results_MMR[, TR:= paste0(round(1/Estimate, digits = 2), " (", round(1/uci, digits = 2), "-", round(1/lci, digits = 2), ")")]

write.table(results_MMR, paste0(results, "aft_results_MMR_timingCMD_CC.csv"), sep = "\t", row.names = T,
            dec = ".")
